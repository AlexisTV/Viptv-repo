...
INCLUDEANARCHY   = wiz.getS('includeanarchy')
INCLUDEBEAUS     = wiz.getS('includebeausplace')
INCLUDEBENNU     = wiz.getS('includebennu')
INCLUDEBONE      = wiz.getS('includebonecrusher')
INCLUDEBOXSET    = wiz.getS('includeboxsetkings')
INCLUDEBUBBLE    = wiz.getS('includebubbles')
...
INCLUDEEXOSHARK  = wiz.getS('includeexoshark')
INCLUDEFLIXAN    = wiz.getS('includeflixanity')
INCLUDEGURZIL    = wiz.getS('includegurzil')
INCLUDEHITLISTS  = wiz.getS('includehitlists')
INCLUDENOTSURE   = wiz.getS('includenotsure')
INCLUDEPIRANHA   = wiz.getS('includepiranha')
INCLUDEPOSEIDON  = wiz.getS('includeposeidon')
INCLUDEPROJM     = wiz.getS('includeprojectm')
INCLUDEREDPILL   = wiz.getS('includeredpill')
INCLUDERESIST    = wiz.getS('includeresistance')
INCLUDEREVEL     = wiz.getS('includerevelations')
INCLUDESALTS     = wiz.getS('includesalts')
INCLUDESALTSHD   = wiz.getS('includesaltshdlite')
INCLUDESALTSRD   = wiz.getS('includesaltsrdlite')
INCLUDESILENT    = wiz.getS('includesilenthunter')
INCLUDESPECTO    = wiz.getS('includespecto')
INCLUDESPORTIE   = wiz.getS('includesportie')
INCLUDESTREAMH   = wiz.getS('includestreamhub')
INCLUDETHEDEL    = wiz.getS('includethedelorean')
INCLUDETINKLEP   = wiz.getS('includetinklepad')
INCLUDETRIANGU   = wiz.getS('includetriangulum')
INCLUDEUKTURK    = wiz.getS('includeukturk')
INCLUDEWTFORK    = wiz.getS('includewtfork')
...
def index():
	...
	if len(BUILDNAME) > 0:
		...
		addDir(Plataforma,'viewbuild',BUILDNAME, themeit=THEME4)
	...
	addDir ('Instalar Plataformas'        ,'builds',   icon=ICONBUILDS,   themeit=THEME1)
	addDir ('Mantenimiento'   ,'maint',    icon=ICONMAINT,    themeit=THEME1)
	if wiz.platform() == 'android' or DEVELOPER == 'true': addDir ('Kodi Forks' ,'apk', icon=ICONAPK, themeit=THEME1)
	...
	addDir ('Que Salvar Antes de Instalar Plataforma'     ,'savedata', icon=ICONSAVE,     themeit=THEME1)
...
def viewBuild(name):
	...
	for version, url, gui, kodi, themefile, icon, fanart, preview, adult, description in match:
		...
		addFile('Instalacion Limpia'   , 'install', name, 'fresh'  , description=description, fanart=fanart, icon=icon, themeit=THEME1)
...
def apkMenu(url=None):
	if url == None:
		...
		addDir ('Official SPMC APKs', 'apkscrape', 'spmc', icon=ICONAPK, themeit=THEME1)
		if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
	if not APKFILE == 'http://':
		if url == None:
...
def maintMenu(view=None):
	...
	if includeall == 'true':
		...
		includeana = 'true' if INCLUDEANARCHY == 'true' else 'false'
		includebea = 'true' if INCLUDEBEAUS == 'true' else 'false'
		includeben = 'true' if INCLUDEBENNU == 'true' else 'false'
		includebon = 'true' if INCLUDEBONE == 'true' else 'false'
		includebox = 'true' if INCLUDEBOXSET == 'true' else 'false'
		includebub = 'true' if INCLUDEBUBBLE == 'true' else 'false'
		...
		includeexs = 'true' if INCLUDEEXOSHARK  == 'true' else 'false'
		includefli = 'true' if INCLUDEFLIXAN == 'true' else 'false'
		includegur = 'true' if INCLUDEGURZIL == 'true' else 'false'
		includehit = 'true' if INCLUDEHITLISTS == 'true' else 'false'
		includenot = 'true' if INCLUDENOTSURE == 'true' else 'false'
		includepir = 'true' if INCLUDEPIRANHA == 'true' else 'false'
		includepos = 'true' if INCLUDEPOSEIDON == 'true' else 'false'
		includepjm = 'true' if INCLUDEPROJM == 'true' else 'false'
		includered = 'true' if INCLUDEREDPILL == 'true' else 'false'
		includeres = 'true' if INCLUDERESIST == 'true' else 'false'
		includerev = 'true' if INCLUDEREVEL == 'true' else 'false'
		...
		includesil = 'true' if INCLUDESILENT == 'true' else 'false'
		includespe = 'true' if INCLUDESPECTO  == 'true' else 'false'
		includespo = 'true' if INCLUDESPORTIE  == 'true' else 'false'
		includesth = 'true' if INCLUDESTREAMH  == 'true' else 'false'
		includethe = 'true' if INCLUDETHEDEL == 'true' else 'false'
		includetin = 'true' if INCLUDETINKLEP  == 'true' else 'false'
		includetri = 'true' if INCLUDETRIANGU == 'true' else 'false'
		includeukt = 'true' if INCLUDEUKTURK == 'true' else 'false'
		includewtf = 'true' if INCLUDEWTFORK == 'true' else 'false'
	...
	addDir ('[B]Herramientas de Limpiezas[/B]'       ,'maint', 'clean',  icon=ICONMAINT, themeit=THEME1)
	if view == "clean" or SHOWMAINT == 'true': 
		addFile('Limpieza total: [COLOR %s][B]%s[/B][/COLOR]' % (COLOR2, wiz.convertSize(totalsize))    ,'fullclean',       icon=ICONMAINT, themeit=THEME3)
		addFile('Limpiar cache: [COLOR %s][B]%s[/B][/COLOR]' % (COLOR2, wiz.convertSize(sizecache))      ,'clearcache',      icon=ICONMAINT, themeit=THEME3)
		addFile('Limpiar Paquetes: [COLOR %s][B]%s[/B][/COLOR]' % (COLOR2, wiz.convertSize(sizepack))    ,'clearpackages',   icon=ICONMAINT, themeit=THEME3)
		addFile('Borrar miniaturas: [COLOR %s][B]%s[/B][/COLOR]' % (COLOR2, wiz.convertSize(sizethumb)) ,'clearthumb',      icon=ICONMAINT, themeit=THEME3)
		addFile('Borrar miniaturas antiguas', 'oldThumbs',      icon=ICONMAINT, themeit=THEME3)
		addFile('Borrar registros de bloqueo',               'clearcrash',      icon=ICONMAINT, themeit=THEME3)
		addFile('Purgar Base de Datos',                'purgedb',         icon=ICONMAINT, themeit=THEME3)
		addFile('Borra Todo, Nuevo Comienzo Kodi desde 0',                    'freshstart',      icon=ICONMAINT, themeit=THEME3)
	...
	addFile('Show [B]ALL[/B] [I][COLOR %s]([/COLOR]Looking Glass[COLOR %s])[/COLOR][/I]  Submenus: %s' % (COLOR1, COLOR1, maint.replace('true',on).replace('false',off)), 'togglesetting', 'showmaint', icon=ICONMAINT, themeit=THEME2)
	...
	if includevid == 'true':
		...
		addFile('[COLOR %s]   ---[/COLOR] Include AnArchy: %s' % (COLOR2, includeana.replace('true',on).replace('false',off)), 'togglecache', 'includeanarchy', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR %s]   ---[/COLOR] Include Beau\'s Place: %s' % (COLOR2, includebea.replace('true',on).replace('false',off)), 'togglecache', 'includebeausplace', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR %s]   ---[/COLOR] Include Bennu: %s' % (COLOR2, includeben.replace('true',on).replace('false',off)), 'togglecache', 'includebennu', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR %s]   ---[/COLOR] Include Bonecrusher: %s' % (COLOR2, includebon.replace('true',on).replace('false',off)), 'togglecache', 'includebonecrusher', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR %s]   ---[/COLOR] Include Boxset Kings: %s' % (COLOR2, includebox.replace('true',on).replace('false',off)), 'togglecache', 'includeboxsetkings', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR %s]   ---[/COLOR] Include Bubbles: %s' % (COLOR2, includebub.replace('true',on).replace('false',off)), 'togglecache', 'includebubbles', icon=ICONMAINT, themeit=THEME3)
		...
		addFile('[COLOR %s]   ---[/COLOR] Include ExoShark: %s' % (COLOR2, includeexs.replace('true',on).replace('false',off)), 'togglecache', 'includeexoshark', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR %s]   ---[/COLOR] Include Flixanity: %s' % (COLOR2, includefli.replace('true',on).replace('false',off)), 'togglecache', 'includeflixanity', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR %s]   ---[/COLOR] Include Gurzil: %s' % (COLOR2, includegur.replace('true',on).replace('false',off)), 'togglecache', 'includegurzil', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR %s]   ---[/COLOR] Include The Hit Lists: %s' % (COLOR2, includehit.replace('true',on).replace('false',off)), 'togglecache', 'includehitlists', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR %s]   ---[/COLOR] Include Not Sure: %s' % (COLOR2, includenot.replace('true',on).replace('false',off)), 'togglecache', 'includenotsure', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR %s]   ---[/COLOR] Include Piranha: %s' % (COLOR2, includepir.replace('true',on).replace('false',off)), 'togglecache', 'includepiranha', icon=ICONMAINT, themeit=THEME3)
		...
		addFile('[COLOR %s]   ---[/COLOR] Include Red Pill: %s' % (COLOR2, includered.replace('true',on).replace('false',off)), 'togglecache', 'includeredpill', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR %s]   ---[/COLOR] Include Resistance: %s' % (COLOR2, includeres.replace('true',on).replace('false',off)), 'togglecache', 'includeresistance', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR %s]   ---[/COLOR] Include Revelations: %s' % (COLOR2, includerev.replace('true',on).replace('false',off)), 'togglecache', 'includerevelations', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR %s]   ---[/COLOR] Include Salts: %s' % (COLOR2, includesal.replace('true',on).replace('false',off)), 'togglecache', 'includesalts', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR %s]   ---[/COLOR] Include Salts HD Lite: %s' % (COLOR2, includeshd.replace('true',on).replace('false',off)), 'togglecache', 'includesaltshdlite', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR %s]   ---[/COLOR] Include Salts RD Lite: %s' % (COLOR2, includesrd.replace('true',on).replace('false',off)), 'togglecache', 'includesaltsrdlite', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR %s]   ---[/COLOR] Include Silent Hunter: %s' % (COLOR2, includesil.replace('true',on).replace('false',off)), 'togglecache', 'includesilenthunter', icon=ICONMAINT, themeit=THEME3)
		...
		addFile('[COLOR %s]   ---[/COLOR] Include Sportie: %s' % (COLOR2, includespo.replace('true',on).replace('false',off)), 'togglecache', 'includesportie', icon=ICONMAINT, themeit=THEME3)
		...
		addFile('[COLOR %s]   ---[/COLOR] Include The DeLorean: %s' % (COLOR2, includethe.replace('true',on).replace('false',off)), 'togglecache', 'includethedelorean', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR %s]   ---[/COLOR] Include TinklePad: %s' % (COLOR2, includetin.replace('true',on).replace('false',off)), 'togglecache', 'includetinklepad', icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR %s]   ---[/COLOR] Include Triangulum: %s' % (COLOR2, includetri.replace('true',on).replace('false',off)), 'togglecache', 'includetriangulum', icon=ICONMAINT, themeit=THEME3)
		...
		addFile('[COLOR %s]   ---[/COLOR] Include What The Fork?: %s' % (COLOR2, includewtf.replace('true',on).replace('false',off)), 'togglecache', 'includewtfork', icon=ICONMAINT, themeit=THEME3)
...
def toggleCache(state):
	cachelist = ['includevideo', 'includeall', 'includeanarchy', 'includebeausplace', 'includebennu', 'includebonecrusher', 'includeboxsetkings', 'includebubbles', 'includecovenant', 'includedeathstreams', 'includeelysium', 'includeexodus', 'includeexoshark', 'includeflixanity', 'includegurzil', 'includehitlists', 'includenotsure', 'includepiranha', 'includeposeidon', 'includeprojectm', 'includeredpill', 'includeresistance', 'includerevelations', 'includesalts', 'includesaltshdlite', 'includesaltsrdlite', 'includesilenthunter', 'includespecto', 'includesportie', 'includestreamhub', 'includethedelorean', 'includetinklepad', 'includetriangulum', 'includeukturk', 'includewtfork', 'includexxxodus']
	titlelist = ['Include Video Addons', 'Include All Addons', 'Include AnArchy', 'Include Beau\'s Place', 'Include Bennu', 'Include Bonecrusher', 'Include Boxset Kings', 'Include Bubbles', 'Include Covenant', 'Include Death Streams', 'Include Elysium', 'Include Exodus', 'Include ExoShark', 'Include Flixanity', 'Include Gurzil', 'Include The Hit Lists', 'Include Not Sure', 'Include Piranha', 'Include Poseidon', 'Include Project M', 'Include Red Pill', 'Include Resistance', 'Include Revelations', 'Include Salts', 'Include Salts HD Lite', 'Include Salts RD Lite', 'Include Silent Hunter', 'Include Specto', 'Include Sportie', 'Include StreamHub', 'Include The DeLorean', 'Include TinklePad', 'Include Triangulum', 'Include UK Turk Playlists', 'Include What The Fork?', 'Include XXX-O-DUS']
